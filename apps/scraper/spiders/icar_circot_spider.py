# ============================================================
# spiders/icip_circot_spider.py — ICAR-CIRCOT Mumbai (DYNAMIC)
# Target: https://circot.icar.gov.in/careers
#
# PAGE STRUCTURE:
# - ICAR institute careers page
# - Likely table or card-based layout for job openings
# - PDF links for advertisements
# - Walk-in interview dates common
#
# PRINCIPLES (same as ACTREC/NEERI/IISER/ICT/PDKV spiders):
...
class IcarCircotSpider(DuplicateStopMixin, scrapy.Spider):
    name = "icar_circot"
    source_name = "ICAR-CIRCOT Official Careers Portal"
    allowed_domains = ["circot.icar.gov.in"]
    start_urls = [
        "https://circot.icar.gov.in/careers",
        "https://circot.icar.gov.in/",  # Warm-up
    ]

    dedup_org = "ICAR_CIRCOT"
    
    # Multi-state / multi-language metadata
    target_state = "maharashtra"  # CIRCOT is in Mumbai, Maharashtra
    ...
    JOB_KEYWORDS = [
        ...
        "young professional", "project", "technical", "contractual",
        "adhoc", "srf", "jrf", "ra"
    ]

    def parse(self, response):
        self.logger.info(f"ICAR-CIRCOT: Parsing {response.url} — status {response.status}")
        
        # If this is homepage warm-up, go to careers page
        if response.url == "https://circot.icar.gov.in/" or response.url == "https://circot.icar.gov.in":
    ...
    def parse_careers_page(self, response):
        """Parse the careers page and extract job openings."""
        self.logger.info(f"ICAR-CIRCOT: Parsing careers page — status {response.status}")

        # Try multiple parsing strategies
        entries = self._find_job_entries(response)
        
        if entries:
            self.logger.info(f"ICAR-CIRCOT: Found {len(entries)} job entries")
            for entry in entries:
                item = self._parse_entry(entry, response)
                if item:
                    yield item
        else:
            self.logger.warning("ICAR-CIRCOT: No job entries found with standard selectors")
    ...
    def _find_job_entries(self, response):
        ...
        if not entries:
            containers = response.css("div, article, section, li, .card, .job, .vacancy")
    ...
    def _parse_entry(self, entry, response):
        ...
        # Deduplication
        dedup_string = f"CIRCOT_{advt_no or title_raw[:40]}"
        ...
        if self.track_duplicate(dedup_hash, label=title_raw, urls=[candidate_url]):
            self.logger.debug(f"CIRCOT: Skipping known entry")
            return None
        ...
        item['org_name'] = "ICAR-Central Institute for Research on Cotton Technology"
        item['org_acronym'] = "ICAR-CIRCOT"
        ...
        item['application_links'] = {
            "official_website": "https://circot.icar.gov.in",
        ...
        item['seo_metadata'] = {
            "meta_title": f"{clean_title} | ICAR-CIRCOT Recruitment {year}",
            "meta_description": f"{clean_title} at ICAR-CIRCOT, Mumbai.",
        }

        self.logger.info(f"CIRCOT: Yielded item: {clean_title[:50]} | PDF: {notification_pdf or 'None'}")
        return item
    ...
    def _scrape_all_links(self, response):
        ...
        for link in response.css("a"):
            ...
            if is_pdf:
                # Direct PDF link
                dedup_hash = hashlib.sha256(f"CIRCOT_PDF_{full_url}".encode()).hexdigest()
